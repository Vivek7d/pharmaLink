...
function ShipmentProps() {
    ...
    if (id) {
      const fetchData = async () => {
        try {
          ...
          // Fetch shipment data by shipmentId
          ...
          if (!shipmentSnapshot.empty) {
            setShipment(shipmentSnapshot.docs[0].data()); // Assuming there's only one document with that shipmentId
          } else {
            setError('No shipment found with the given ID');
            return; // Stop further execution if no shipment found
          }

          // Fetch drivers data by shipmentId
          const driversQuery = query(
            collection(db, 'drivers'),
            where('shipmentId', '==', id) // Searching for drivers with the same shipmentId
          );
          ...
          if (!driversSnapshot.empty) {
            ...
            // Assuming there's only one driver expected for the shipment
            if (driversData.length === 1) {
              setDrivers(driversData[0]); // Set the single driver
            } else {
              setError('Expected one driver, but found multiple drivers.');
            }
          } else {
            setError('No drivers found for the given shipmentId');
          }
  ...
  }, [id]); // Dependency array, will rerun whenever `id` changes
    ...
    <div className={`${poppins.className} flex h-screen relative`}>
      ...
      <div className="flex-1 h-full">
        <Map
          ...
          startAddress={shipment.supplierLocation}
          endAddress={shipment.hospitalLocation}
        />
      </div>
        ...
        <div className="flex justify-start items-start flex-col py-5">
          ...
          <div className="space-y-5 w-full flex flex-col justify-center items-start py-5">
            ...
            <div className="flex justify-between items-center space-x-2">
              ...
              <p className="">{shipment.supplier}</p>
            </div>
